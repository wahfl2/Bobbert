...
public class FakeChunkManager {
    ...
    private static final ExecutorService loadExecutor = Executors
        .newFixedThreadPool(8, new DefaultThreadFactory("bobby-loading", true));
    ...
    public FakeChunkManager(WorldClient world, ChunkProviderClient clientChunkManager) {
        ...
        Path storagePath = client.mcDataDir.toPath()
            .resolve(".bobby")
    ...
    public void update(BooleanSupplier shouldKeepTicking) {
        ...
        long unloadTime = time - BobbertConfig.unloadDelaySecs * 1000L;
        ...
        ObjectIterator<LoadingJob> loadingJobsIter = this.loadingJobs.values()
            .iterator();
    ...
    private class LoadingJob implements Runnable {
        ...
        public void run() {
            ...
            result = Optional.ofNullable(loadTag(x, z))
                .map(
                    it -> it.getRight()
                        .deserialize(new ChunkPos(x, z), it.getLeft(), world));
        }
